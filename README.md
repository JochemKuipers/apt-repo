...
Unofficial packages of howdy-next, adguard-tray, skills-manager, uniwill-laptop, dawnpro-gui, and thrm.

```sh
curl -fsSL https://jochemkuipers.github.io/apt-repo/jochem.sources \
  | sudo tee /etc/apt/sources.list.d/jochem.sources
sudo apt update
sudo apt install howdy-next adguard-tray skills-manager uniwill-laptop dawnpro-gui thrm
```
